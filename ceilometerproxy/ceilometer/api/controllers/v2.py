...
class AlarmController(rest.RestController):
    ...
    def _get_cascaded_alarm(self, cascading_alarm):
        desc = json.loads(cascading_alarm.description)
    ...
    def _merge_alarm_attributes(self, cascading, cascaded):
        ...
        kwargs['alarm_id'] = cascading.alarm_id
        return Alarm(**kwargs)
    ...
    def get(self):
        """Return this alarm."""
        alarm = self._alarm()
        cascaded_alarm = self._get_cascaded_alarm(alarm)
        return self._merge_alarm_attributes(alarm, cascaded_alarm)
    ...
    def put(self, data):
        ...
        for query in cascaded_alarm.rule['query']:
            if query['field'] in ('resource_id', 'resource'):
                query['value'] = resource_id
    ...
    def delete(self):
        ...
        try:
            _cmclient.alarms.delete(cascaded_alarm_id)
        except exceptions.NotFound as e:
            LOG.info(("Failed to delete cascaded alarm because %s, "
                      "clear data in cascading level"), e)
    ...
    def put_state(self, state):
        ...
        cascaded_alarm_id = json.loads(alarm.description)['cascaded_alarm_id']
        cmclient = self._get_cascaded_cm_client(alarm)
        cmclient.alarms.set_state(cascaded_alarm_id, state)
        return alarm.state
...
class AlarmsController(rest.RestController):
    ...
    def get_all(self, q=None):
        ...
        kwargs = _query_to_kwargs(q,
                                  ...
                                  allow_timestamps=False)
        alarms = []
        for alarm in pecan.request.alarm_storage_conn.get_alarms(**kwargs):
            try:
                ...
                alarms.append(alarm)
        return [Alarm.from_db_model(m) for m in alarms]
